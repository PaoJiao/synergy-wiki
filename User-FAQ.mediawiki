...
You can re-enable XOrg as a temporary workaround by doing the following:
Edit `/etc/gdb/custom.conf` and change line 5 per line 4. You'll then need to reboot. This has the machine fall back onto Xorg for the session manager.
